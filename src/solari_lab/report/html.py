...
def _where(results: list[Result], env: dict[str, Any]) -> str:
    """Where the numbers were measured from, and what that means for latency."""
    doc = next((r for r in results if r.name == "doctor"), None)
    floor = doc.data.get("network_floor_s") if doc else None
    bench = next((r for r in results if r.name == "bench"), None)
    create = bench.data.get("phases", {}).get("create", {}).get("p50") if bench else None
    cold = doc.data.get("network_cold_s") if doc else None
    floor_txt = (
        f"<b>{e(ms(floor))}</b> per round trip on a warm connection"
        + (f", {e(ms(cold))} with a fresh TLS handshake" if cold else "")
        if floor
        else "not measured"
    )
    ratio = ""
    if floor and create:
        ratio = (
            f" Session create p50 was <b>{e(ms(create))}</b>, so most of it is the round trip, not Solari."
        )
    return (
        f"<section><h2><span class=mark>◎</span>Where this was measured from</h2>"
        f"<p class=summary>Client on {e(env.get('os'))}, gateway region <b>{e(env.get('region'))}</b>, the only region Solari offers today.</p>"
        f"<p style='margin:0 0 8px'>Network floor from this client to the gateway: {floor_txt}.{ratio} "
        f"Solari's published latencies are measured inside the region. From Europe, the Middle East or Africa the floor dominates every number on this page, "
        f"and an EU region would roughly halve it.</p></section>"
    )
...
def write_html(path: Path, results: list[Result], env: dict[str, Any]) -> None:
    ...
    for r in results:
        ...
        if r.name == "doctor":
            ...
            if r.data.get("network_floor_s"):
                tiles.append(("network floor", ms(r.data["network_floor_s"])))
        if r.name == "isolation":
...
<div class=grid>{tiles_html}</div>{_where(results, env)}{"".join(sections)}
